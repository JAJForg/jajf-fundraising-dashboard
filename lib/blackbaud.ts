...
async function getAccessToken(): Promise<string> {
  ...
  // Blackbaud issues a new refresh token on every use and invalidates the
  // old one — save it immediately so the next request doesn't fail.
...
export async function fetchLiveSnapshot(): Promise<FundraisingSnapshot> {
  ...
  // JAJF's structure in RE NXT:
  //  - Fund "Cash Fund" contains three Appeals: Cash, Grant, Pledge
  //  - Fund "Gift-in-Kind" is entirely separate
  // Matching is fund + appeal name (case-insensitive "contains"), with the
  // gift `type` field as a fallback for any gifts not yet coded with an
  // appeal. Adjust the string matches below if JAJF's exact fund/appeal
  // names in RE NXT differ from these.
  ...
  // Every gift posted this calendar month, split into three categories and
  // sorted newest first, using the same Fund/Appeal matching as the donor
  // lists above.
