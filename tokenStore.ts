...
export async function GET(req: NextRequest) {
  ...
  const res = htmlPage(
    `<h1>Connected to Raiser's Edge NXT</h1>
     <p>If Vercel KV is set up (see README), this token is now saved automatically — you shouldn't need to do anything else. As a backup, here's the value in case KV isn't configured yet: copy it into Vercel's environment variables as <code>BLACKBAUD_REFRESH_TOKEN</code>, then redeploy.</p>
     <div class="label">BLACKBAUD_REFRESH_TOKEN</div>
     <div class="box">${tokenData.refresh_token}</div>
     <p>Once that's saved (automatically via KV, or manually in Vercel), <strong>/team</strong> and <strong>/board</strong> will automatically switch from sample data to live data — no other changes needed.</p>
